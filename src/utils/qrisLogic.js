...
function generateDynamicQR(baseQR, nominal) {
  ...
  // Buang 4 char CRC lama
  const noCRC = baseQR.slice(0, -4);

  // Ubah ke QRIS Dinamis (Point of Initiation Method)
  // 11 = Static, 12 = Dynamic
  const makeDynamic = noCRC.replace("010211", "010212");

  // Pecah bagian sebelum dan setelah "5802ID" (Country Code Indonesia)
  const split = makeDynamic.split("5802ID");

  if (split.length < 2) {
    throw new Error(
      "QRIS tidak valid (tag 5802ID tidak ditemukan). Pastikan ini QRIS Indonesia."
    );
  }

  // Buat tag 54 (Transaction Amount)
  const tag54 =
...
module.exports = { generateDynamicQR };
